...
def create_address(post:Data,db: Session = Depends(get_db)):
    user =  models.Address(address_id=post.address_id,full_name=post.full_name,house_no=post.house_no,road_name_Area=post.road_name_Area,landmark=post.landmark,country=post.country,city=post.city,latitude=post.latitude,longitude=post.longitude)    
    db.add(user)
    db.commit()
    db.refresh(user)
